...
export const addTeamMember = async (customerId: string, role: Role): Promise<{ success: boolean }> => {
  console.log(`Added customer ${customerId} as team member with role ${role}`);
  return { success: true };
};

export const removeTeamMember = async (teamMemberId: string): Promise<{ success: boolean }> => {
  console.log(`Removed team member with ID ${teamMemberId}`);
  return { success: true };
};

export const toggleTeamMemberStatus = async (teamMemberId: string, isActive: boolean): Promise<{ success: boolean }> => {
  console.log(`Toggled team member ${teamMemberId} active status to ${isActive}`);
  return { success: true };
};

export const fetchAddMemberCustomers = async (timeRange: string = 'today', filter: CustomerFilter = {}): Promise<AddMemberCustomer[]> => {
  const verifiedCustomers = await fetchVerifiedCustomers(timeRange, filter);

  const customers: AddMemberCustomer[] = verifiedCustomers.map(customer => ({
    id: customer.id,
    ...
    status: customer.status === 'SUCCESSFUL' ? 'Active' : 'Inactive',
  }));

  return customers;
};

export const fetchAuditNotification = async (): Promise<Notification> => {
  return { message: 'Anna just viewed accounts' };
};

export const fetchAuditLogs = async (): Promise<AuditLog[]> => {

  return [
    { id: '1-0', member: 'Jeremiah Omonefe', roles: ['Owner', 'Admin'], activity: 'View Lookup', date: '08/04/25 4:30pm' },
    { id: '1-1', member: 'Jeremiah Omonefe', roles: ['Owner', 'Admin'], activity: 'View Lookup', date: '08/04/25 4:30pm' },
    { id: '1-2', member: 'Jeremiah Omonefe', roles: ['Owner', 'Admin'], activity: 'View Lookup', date: '08/04/25 4:30pm' },
    { id: '2-0', member: 'Anna Smith', roles: ['Admin'], activity: 'View Lookup', date: '08/04/25 4:30pm' },
    { id: '2-1', member: 'Anna Smith', roles: ['Admin'], activity: 'View Lookup', date: '08/04/25 4:30pm' },
    { id: '2-2', member: 'Anna Smith', roles: ['Admin'], activity: 'View Lookup', date: '08/04/25 4:30pm' },
  ];
};
...
export const fetchRates = async (): Promise<Rate[]> => {
  ...
  return [
    { id: '1', label: 'Config MIDEN Rate', rate: '$1 → NGN1,800.98' },
    { id: '2', label: 'Config MIDEN Rate', rate: '$1 → NGN1,800.98' },
    { id: '3', label: 'Config MIDEN Rate', rate: '$1 → NGN1,800.98' },
    { id: '4', label: 'Config MIDEN Rate', rate: '$1 → NGN1,800.98' },
    { id: '5', label: 'Config MIDEN Rate', rate: '$1 → NGN1,800.98' },
    { id: '6', label: 'Config MIDEN Rate', rate: '$1 → NGN1,800.98' },
  ];
};

export const updateRate = async (id: string, newRate: number): Promise<{ success: boolean }> => {
  console.log(`Updated rate with ID: ${id} to new rate: ${newRate}`);
  return { success: true };
};
...
export const updateUserProfile = async (profile: UserProfile): Promise<{ success: boolean }> => {
  console.log('Updated user profile:', profile);
  return { success: true };
};

export const changePassword = async (newPassword: string): Promise<{ success: boolean }> => {
