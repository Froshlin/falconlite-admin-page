...
import { CardFilter, FilterOptions, TimeRangeFilterProps } from '@/types/types';
...
export const TimeRangeFilter = ({
  ...
  const [filterOptions, setFilterOptions] = useState<FilterOptions>({ currencyTypes: [], transactionTypes: [] });

  // Fetch filter options (currency types and transaction types) on component mount
  useEffect(() => {
  ...
  const handleApplyFilter = () => {
    if (onFilterChange) {
      onFilterChange(filter); // Pass the filter to the parent component
    }
        ...
        <div className="fixed top-5 inset-0 flex items-center justify-center z-2100">
          ...
          <div className="relative bg-white rounded-lg w-full max-w-[604px] md:ml-auto md:mr-10 shadow-lg">
            <div className="flex justify-between items-center border-b border-[#d7d9db] pt-12 pb-4 px-6">
              <h2 className="text-[16px] font-medium text-[#8B9DA4]">Filter by</h2>
              <button
                onClick={() => setIsModalOpen(false)}
            ...
            <div className="p-6 space-y-6">
              {/* Date Range Buttons */}
              <div>
                <h3 className="text-[13px] font-medium text-[#0A0E3F] mb-2">Choose Data Range</h3>
                <div className="flex gap-2">
                  <button
                    onClick={() => handleDateRangeClick('last-week')}
                    className={`cursor-pointer px-4 py-2 text-sm rounded border ${filter.dateRange === 'last-week' ? 'bg-[#36C6F3] text-white' : 'bg-[rgba(135,140,141,0.1)]'} text-[#878C8D]`}
                  >
                    Last Week
                  </button>
                  <button
                    onClick={() => handleDateRangeClick('last-month')}
                    className={`cursor-pointer px-4 py-2 text-sm rounded border ${filter.dateRange === 'last-month' ? 'bg-[#36C6F3] text-white' : 'bg-[rgba(135,140,141,0.1)]'} text-[#878C8D]`}
                  >
                    Last Month
                  </button>
                  <button
                    onClick={() => handleDateRangeClick('last-2-month')}
                    className={`cursor-pointer px-4 py-2 text-sm rounded border ${filter.dateRange === 'last-2-month' ? 'bg-[#36C6F3] text-white' : 'bg-[rgba(135,140,141,0.1)]'} text-[#878C8D]`}
                  >
                    Last 2 Month
                  </button>
                  <button
                    onClick={() => handleDateRangeClick('last-3-month')}
                    className={`cursor-pointer px-4 py-2 text-sm rounded border ${filter.dateRange === 'last-3-month' ? 'bg-[#36C6F3] text-white' : 'bg-[rgba(135,140,141,0.1)]'} text-[#878C8D]`}
                  >
                    Last 3 Month
                  </button>
                </div>
              ...
              <div className="grid grid-cols-2 w-full gap-4">
                <div>
                  <label className="text-[13px] font-medium text-[#0A0E3F] mb-2 block">Start Date</label>
                  <DatePicker
                ...
                <div>
                  <label className="text-[13px] font-medium text-[#0A0E3F] mb-2 block">End Date</label>
                  <DatePicker
              ...
              {/* Card ID */}
              <div>
                <label className="text-[13px] font-medium text-[#0A0E3F] mb-2 block">Card ID</label>
                <input
                  type="text"
                  placeholder="Enter ID"
                  value={filter.cardId}
                  onChange={(e) => setFilter({ ...filter, cardId: e.target.value })}
              ...
              {/* Currency Type */}
              <div>
                <label className="text-[13px] font-medium text-[#0A0E3F] mb-2 block">Currency Type</label>
                <div className="relative">
                  <select
                    value={filter.currencyType}
                    onChange={(e) => setFilter({ ...filter, currencyType: e.target.value })}
              ...
              {/* Transaction Type */}
              <div>
                <label className="text-[13px] font-medium text-[#0A0E3F] mb-2 block">Transaction Type</label>
                <div className="relative">
                  <select
