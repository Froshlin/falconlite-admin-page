...
import { PndCustomerTableProps } from '@/types/types';
import { CustomerActions } from './CustomerActions';

export const PndCustomerTable = ({ customers, currentPage, itemsPerPage }: PndCustomerTableProps) => {
        ...
        {paginatedCustomers.length === 0 ? (
          ...
          paginatedCustomers.map((customer, index) => (
            <TableRow key={customer.id} className={index % 2 === 0 ? 'bg-[#F8FCFF] border-b-0' : 'bg-[#FFFFFF] border-b-0'}>
              ...
              <CustomerActions customer={customer} tableType="pnd" onCustomerUpdate={() => window.location.reload()} />
              </TableCell>
            </TableRow>
          ))
        )}
